...
const Navbar = () => {
    ...
    return (
        <header className="top-0 z-50 bg-white px-5 md:px-27 ">
            <div className="flex items-center justify-between py-6">
                <img
                    src={logo}
                    alt="Logo Newlook Browlash"
                    className="h-8 w-auto"
                />

                {/* Desktop nav */}
                <nav className="hidden md:flex gap-4 text-white font-medium">
                    {["Home", "Layanan", "Portfolio", "Lokasi", "Hubungi"].map(
                        (label, index) => (
                            <div
                                className="transition duration-400 hover:bg-gradient-to-br from-[#9C6F22] via-[#F4D88D] to-[#9C6F22] rounded-full p-1"
                                key={index}
                            >
                                <button
                                    key={index}
                                    ...
                                    className="px-5 py-1 rounded-full bg-white text-black hover:bg-[#1D1D1D] hover:text-white transition duration-200 outline-none focus:outline-none"
                                >
                                    {label}
                                </button>
                            </div>
